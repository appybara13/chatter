...
/// Either a [Line] or a Vec of [Choice]s.
...
pub use chat::ChatPosition;

/// A Vec of choices represents a set of possible options, one of which can be taken.
/// They allow the dialogue to branch.
pub use choices::Choice;

/// Represents a line of dialogue. A [Chat] that consists of just lines and no choices
/// will play the same every time.
pub use line::Line;

/// A trait that allows more ergonomic checking tags on a [Tagged] type.
pub use tag::Tag;

/// Both [Line] and [Choice] can be tagged.
pub use tag::Tagged;

/// The macro for parsing chatter script into a [Chat].
/// The script must be a raw string literal, ``r#"SCRIPT"#``.
pub use chatter_macros::chatter;
